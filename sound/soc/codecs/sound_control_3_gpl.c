 ...
 *
 * Credits to "robcore" for fixing the "Wrong Default-Values at Boot" issue of the Driver.
 *
 * Updated to v3.9 by Shoaib Anwar a.k.a Shoaib0597 <shoaib0595@gmail.com> :
 * 1. Fixed the Registry and Cache Size according to the Gains (MSM8916 supports fewer Gains).
 * 2. Added an Enable/Disable Toggle with a Smart Workaround (this allows full compatibility with Kernel Adiutor-MOD).
 * 3. Altered the Formatting of the Codes (looks cleaner and more beautiful now).
 *
...
int snd_hax_reg_access(unsigned int reg)
{
	...
	{      
		   ...
		   if ((enable_fs > 0) && (snd_ctrl_locked > 0))
		      ret = 0;
		   break;
		   /* Cam-Corder-Mic and Call-Mic Gain */
	       case MSM8X16_WCD_A_CDC_TX1_VOL_CTL_GAIN:
	       case MSM8X16_WCD_A_CDC_TX2_VOL_CTL_GAIN:
		   if ((enable_fs > 0) && (snd_ctrl_locked > 0))
		      ret = 0;
...
{
        return sprintf(buf, "%u\n", msm8x16_wcd_read(fauxsound_codec_ptr, MSM8X16_WCD_A_CDC_TX1_VOL_CTL_GAIN));
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	snd_ctrl_locked = 0;
	msm8x16_wcd_write(fauxsound_codec_ptr, MSM8X16_WCD_A_CDC_TX1_VOL_CTL_GAIN, lval);
	snd_ctrl_locked = 2;
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	snd_ctrl_locked = 0;
	msm8x16_wcd_write(fauxsound_codec_ptr, MSM8X16_WCD_A_CDC_TX2_VOL_CTL_GAIN, lval);
	snd_ctrl_locked = 2;
