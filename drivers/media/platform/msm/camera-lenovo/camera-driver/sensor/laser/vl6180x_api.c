...
int VL6180x_UpscaleInitPatch0(VL6180xDev_t dev);
#define   VL6180x_UPSCALE_INIT_PATCH0  VL6180x_UpscaleInitPatch0
...
int8_t 	user_offset_calib=0;
int16_t user_xtalk_calib=0xc; //change by lijk3 for xtalk
 ...
 */
static const uint16_t UpperLimitLookUP[]  ROMABLE_DATA ={ 255, 510, 765}; /* lookup table for scaling->limit  1x2x3x */
/**
...
static int _filter_GetResult(VL6180xDev_t dev, RangeFilterResult_t *pData);
...
int VL6180x_StaticInit(VL6180xDev_t dev){
    ...
    if( _GetUpscale(dev) == 1 && !(VL6180x_UPSCALE_SUPPORT<0))
        status=VL6180x_RangeStaticInit(dev);
    else
        status=VL6180x_UpscaleStaticInit(dev);
...
int VL6180x_RangePollMeasurement(VL6180xDev_t dev, VL6180x_RangeData_t *pRangeData)
{
    ...
    if(status){
        pr_err("%s:%d VL6180x_RangeClearInterrupt fail \n", __func__, __LINE__);
        return -1;
    }
#endif
...
//        }
//        if( IntStatus.status.Error !=0 ){
//          printk("%s:%d GPIO int Error report %d",__func__, __LINE__,(int)IntStatus.val);
//          status = RANGE_ERROR;
//          break;
//        }
//        else
...
//    }
 ...
 /* //![single_shot_snipet] */
    ...
    if( ClrStatus ){
        pr_err("VL6180x_RangeClearInterrupt fail");
    ...
    if( !status ){
        ...
        if( !status ){
            ...
            if( _IsWrapArroundActive(dev) ){
                status=_filter_GetResult(dev, &pRangeData->FilteredData);
                if( !status){
...
int VL6180x_UpscaleRegInit(VL6180xDev_t dev)
{
    ...
    VL6180x_WrByte( dev, 0x00f5, 0x02);
    VL6180x_WrByte( dev, 0x00d9, 0x05);
#ifdef VL6180x_UPSCALE_INIT_PATCH0
    VL6180x_UPSCALE_INIT_PATCH0(dev);
#else
    VL6180x_WrByte( dev, 0x00db, 0xce);
    VL6180x_WrByte( dev, 0x00dc, 0x03);
    VL6180x_WrByte( dev, 0x00dd, 0xf8);
    ...
    /*+begin:wanglz10 add. STM Hanzhy release setting 2015/03/13 */
    VL6180x_WrByte( dev, 0x00bb, 0x44);
    VL6180x_WrByte( dev, 0x0025, 0x42);
    VL6180x_WrByte( dev, 0x0026, 0x00);
    VL6180x_WrByte( dev, 0x0027, 0x40);
    VL6180x_WrByte( dev, 0x002d, 0x12);
    VL6180x_WrByte( dev, 0x001c, 0x3f);
    /*+end:wanglz10*/
//    VL6180x_WrByte( dev, SYSRANGE_RANGE_CHECK_ENABLES, 0x10);
    return 0;
}
#else
...
int VL6180x_UpscaleSetScaling(VL6180xDev_t dev, uint8_t scaling)
{
	...
	pr_err("%s:%d enter VL6180x_UpscaleSetScaling\n",__func__, __LINE__ );
    ...
    if( scaling>=min_scaling  && scaling<= max_scaling ){
		...
		if(status<0)
		{
			pr_err("%s:%d write range scaler failed status %d\n",__func__, __LINE__, status );
		...
		if(status<0)
		{
			pr_err("%s:%d SYSRANGE_PART_TO_PART_RANGE_OFFSET failed status %d\n",__func__, __LINE__, status );
...
static int  VL6180x_UpscaleStaticInit(VL6180xDev_t dev)
{
    ...
    do{
		/* remove the id, revision checking
        status = VL6180x_RdByte(dev, IDENTIFICATION_MODEL_ID, &ModuleID);
        if( status){
            pr_err("rd IDENTIFICATION_MODEL_ID fail");
            break;
        }
        status=VL6180x_RdByte(dev, IDENTIFICATION_MODULE_REV_MAJOR, &RevMaj);
        if( status){
            pr_err("rd IDENTIFICATION_MODULE_REV_MAJOR fail");
            break;
        }
        status=VL6180x_RdByte(dev, IDENTIFICATION_MODULE_REV_MINOR, &RevMin);
        if( status){
            pr_err("rd IDENTIFICATION_MODULE_REV_MINOR fail");
            break;
        }
		*/
        /*if ( ModuleID  == 0xb4 && RevMaj==0x01 && RevMin>=0x02)*/{
            ...
            if( status){
                pr_err("VL6180x_RangeSetEceState fail");
                break;
            }
#endif
        }
		/*
            ...
            pr_err("feature not supported on module  %x r%x.%x", ModuleID, RevMaj, RevMin );
            status=NOT_SUPPORTED;
        }*/
    } while(0);
    if( !status){
...
int VL6180x_UpscaleInitPatch0(VL6180xDev_t dev){
    ...
    if( CalValue ==0  ){
        ...
        CalValue= 0x00CE03F8;
        status=VL6180x_WrByte(dev, 0xDA, 0x00);
        status=VL6180x_WrByte(dev, 0xDB, 0xCE);
        status=VL6180x_WrByte(dev, 0xDC, 0x03);
        status=VL6180x_WrByte(dev, 0xDD, 0xF8);
    }
    else
    {
        status=VL6180x_WrByte(dev, 0xDA, (CalValue>>24)&0xff);
...
int _filter_GetResult(VL6180xDev_t dev, RangeFilterResult_t *pData) {
    ...
    do {
        ...
        status = VL6180x_RdByte(dev, RESULT_RANGE_VAL, &u8);
        if (status) {
            VL6180x_ErrLog("RESULT_RANGE_VAL rd fail");
            break;
        }
        dist = (uint16_t) u8 * scaler;
        ...
        status = VL6180x_RdDWord(dev, RESULT_RANGE_REFERENCE_SIGNAL_COUNT, &m_refSignalCount);
        if (status) {
        ...
        status = VL6180x_RdDWord(dev, RESULT_RANGE_REFERENCE_AMB_COUNT, &m_refAmbientCount);
        if (status) {
            VL6180x_ErrLog("RESULT_RANGE_REFERENCE_AMB_COUNTrd fail");
            break;
        }
        ...
        m_refAmbientRate = (m_refAmbientCount * cDllPeriods * 1000) / calcConvTime;

        FilteredRange = _filter_Start(dev, dist, (m_rawRange_mm * scaler), m_rtnSignalRate, m_rtnAmbientRate, errorCode);
        if( FilteredRange<0 ){
            status = -1;
            break;
        }
        pData->range_mm= FilteredRange;
        pData->rawRange_mm = m_rawRange_mm * scaler;
        pData->rtnRate = m_rtnSignalRate;
        pData->refRate = m_refSignalRate;
        pData->rtnAmbRate = m_rtnAmbientRate;
        pData->refAmbRate = m_refAmbientRate;
        pData->rtnConvTime = m_rtnConvTime;
        pData->refConvTime = m_refConvTime;
    } while (0);
//	pr_err("\n pData->range_mm %d\n",pData->range_mm);
    return status;
}
...
static int _DMax_Compute(VL6180xDev_t dev, VL6180x_RangeData_t *pRange){
    ...
    rtnAmbRate = pRange->FilteredData.rtnAmbRate;
